...
function add(paramOne, paramTwo){
  const boolTwo = Number(paramOne);
  const boolThree = Number(paramTwo);
  return paramOne + paramTwo;
};

let sum = add(1, 2);
console.log(`This is the answer for problem four ${sum}`);
...
const exclaimTwoTwo = str => `${str.toUpperCase} !!!`;
...
function nameCheck(nameName){
  if(nameName === "Steven"){
    console.log("What is up Steven?")
  } else if(nameName === "Bryan"){
    console.log("Hey Bryan!")
  } else { 
      console.log(`Cool name ${nameName}!`)
  }
}
...
function printAllNames(anArrayOfNames){
  for(let i = 0; i < anArrayOfNames.length; i++){
    console.log(anArrayOfNames[i])
  }
}
...
function chocolateFrogs(gold){
  //return amount of grogs I can purchase
  return gold / 3;
}; 

const totalFrogs = chocolateFrogs(21);
...
function chocolateFrogsTwo(gold){
